...
if (!empty($_SESSION['admin'])) {
    ...
    if (!empty($_GET['userList'])) {
        ...
        if (!empty($_POST['pass'])) {
            $pass = md5($_POST['pass']); // Update password using md5()
            $sql = 'UPDATE login SET user=?, pass=?, role=? WHERE id_member=?';
            $row = $config->prepare($sql);
            $row->execute([$user, $pass, $role, $id_member]);
        } else {
            $sql = 'UPDATE login SET user=?, role=? WHERE id_member=?';
            $row = $config->prepare($sql);
            $row->execute([$user, $role, $id_member]);
        }

        if (!empty($gambar)) {
            // Get the image file and move it to the images folder
            $sql = 'SELECT gambar FROM member WHERE id_member = ?';

            $row = $config->prepare($sql);
            $row->execute([$id_member]);
            $currentImage = $row->fetchColumn();
            ...
            // Update member data with gambar path
            $sql = 'UPDATE member SET nm_member=?, alamat_member=?, telepon=?, email=?, NIK=?, gambar=? WHERE id_member=?';
            $row = $config->prepare($sql);
            $row->execute([$nm_member, $alamat_member, $telepon, $email, $NIK, $xx . "_" . basename($_FILES['gambar']['name']), $id_member]);
        } else {
            // Update member data without gambar path
            $sql = 'UPDATE member SET nm_member=?, alamat_member=?, telepon=?, email=?, NIK=? WHERE id_member=?';
            $row = $config->prepare($sql);
            $row->execute([$nm_member, $alamat_member, $telepon, $email, $NIK, $id_member]);
        }

        echo '<script>window.location="../../index.php?page=userList&uid=' . $id . '&success-edit=edit-data"</script>';
    }
